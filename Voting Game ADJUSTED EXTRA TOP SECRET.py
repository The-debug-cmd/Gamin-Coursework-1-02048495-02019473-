...
def payoff(value, List):
    ...
    #adjust payoffs
    adjust = norm.pdf(List[value],scale = 2, loc = 5.5)
        
    #if it is all the same
    if len(Set) == 1:
        return adjust*10/List.count(List[value])
    # check if it is at the start
    elif value == 0 or Index == 0:
        # = score before, and score to next
        return adjust*(List[value] - (List[value] - Set[Index + 1 ] + 1)/2)/List.count(List[value])
    elif value == len(List) - 1 or Index == len(Set) - 1 :
        # = score to end, and score to before
        return adjust*((10-List[value]+1) + (List[value] - Set[Index - 1 ] - 1)/2)/List.count(List[value])
    else:
        # score at value, score to before, score to next
        return adjust*((List[value] - Set[max(Index - 1,0) ] - 1)/(2) - (List[value] - Set[min(Index + 1, len(Set)-1) ] + 1)/(2) + 1)/List.count(List[value])
...
def display(List, See = True):
    ...
    if See == False:
        ...
        print(f"The number of symmtric equilibria for n = {N} is {symmtros}.")
